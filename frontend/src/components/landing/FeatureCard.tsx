...
export const FeatureCard = ({ title, description, icon, featured, featuredImage, delay = 0 }: FeatureCardProps) => (
  <div className={`bg-surface-card border border-border-subtle rounded-xl p-6 md:p-10 transition-all duration-[350ms] ease-[cubic-bezier(0.16,1,0.3,1)] relative overflow-hidden group reveal ${featured ? 'col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-[auto_1fr] gap-6 md:gap-8 items-center bg-surface-muted border-emerald-700/15' : ''} ${delay ? `reveal-delay-${delay}` : ''}`}>
    ...
    <div className="hover:border-border-hover hover:-translate-y-1 relative z-[1]">
      {featured && featuredImage && (
        <img src={featuredImage} alt={title} className="w-16 md:w-[100px] h-16 md:h-[100px] object-contain" />
      )}
      {icon && (
