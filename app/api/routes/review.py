...
from app.db.session import AsyncSessionLocal, get_database
from sqlalchemy.ext.asyncio import AsyncSession
from app.db.repositories.postgres import save_chapter, save_summary
...
async def get_review(
    ...
    if not pending_tasks:
        raise HTTPException(
            status_code=400, detail="No pending interrupt for this thread"
        )
    ...
    result = await app.ainvoke(
        Command(
            resume={
                ...
                "revised_chapter_text": body.revised_chapter_text,
            }
        ),
        config=config,
    )
        ...
        async with AsyncSessionLocal() as session:
            ...
            latest = next(
                (
                    ...
                    if s["chapter_number"] == result.get("chapter_number")
                ),
                None,
            )
            if latest:
